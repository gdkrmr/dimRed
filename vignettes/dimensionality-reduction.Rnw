...
\abstract{ %
  ...
  objectives and hence (dis)advantages, resulting low-dimensional data
  ...
  \CRANpkg{coRanking} provides the functionality for assessing DR methods in the
  ...
  Here, we outline the \CRANpkg{dimRed} and \CRANpkg{coRanking} packages and
...
just to name a few. %
Either way choosing an inadequate method may imply that much of the underlying structure
remains undiscovered. %
Often the structures of interest in a data set can be well represented by less
dimensions than exist in the original data. %
Data compression of this kind has the additional benefit of making the encoded
information better conceivable to our brains and for further analysis tasks
...
region over the Pacific is often linked to the El Ni\~no Southern Oscillation,
...
ICA), divergence of statistical distributions (SNE family), the reconstruction
...
problem'', i.e.\ a sphere in high dimensional space has a much larger volume
...
There are a number of software package that provide collections of methods: In
...
\citep{van_der_maaten_dimensionality_2009, arenas-garcia_kernel_2013} and the Shogun
...
languages (including R, but the installation is everything but simple, i.e.\
...
This paper presents \CRANpkg{dimRed} and \CRANpkg{coRanking}, both are are
software package in the popular programming language R and
provide a standardized interface to dimensionality reduction methods and quality
metrics for embeddings using the S4 class system making the package packages
both easy to use and to extend.

The goal is to enable researchers who may not necessarily be experts in DR to
apply the methods in their own work to get and objectively identify suitable
methods. %
This paper aims to an overview of the methods collected in the package and how
to use the packages. %
...
These observations may be transformed previously to the dimensionality reduction
...
with $q \ll p$. %
Some methods provide an explicit mapping $f(x'_i) = y_i$ and some even offer a
...
For some methods pairwise distances between points are needed, then $d_{ij} =
d(x_{i}, x_{j})$ and $\hat{d}_{ij} = d(y_i, y_j)$ where $d$ is some appropriate
distance function.

When referring to \code{functions} in the \CRANpkg{dimRed} package or base R simply the
function name is mentioned, functions from other packages are
referenced with \code{package::function}.
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    in bold face are implemented in \CRANpkg{dimRed}.
...
method in \CRANpkg{dimRed} but rather to provide a general idea on how the
methods work; %
...
\CRANpkg{dimRed} tries to make the optimization process for parameters as easy as
possible, but if possible the parameter space should be narrowed down using
prior knowledge. %
Often decisions can be made based on theoretical knowledge, e.g.\ sometimes an
analysis requires data to be kept in their original scales and sometimes this is
exactly what has to be avoided (e.g.\ when comparing different physical
units). %
...
We try to give the computational complexity of the methods but because of the
...
in R often will tend to be slow compared to methods shiped with efficient
implementations in a compiled language. %
Methods where most of the computing time is spent for eigenvalue decomposition
do have very efficient implementations because R uses optimized linear algebra
...
dimensions and dates back to \citet{pearson_lines_1901}. PCA finds a linear
...
scales $\mathcal{O}(np^2 + p^3)$, i.e.\ it scales linearly with the number of
...
scaled to zero mean and unit variance, it depends on the use case if this is
...
The \CRANpkg{dimRed} package wraps around \code{prcomp}.
...
The idea behind kPCA is to maps the data into a high dimensional space using a
possibly non-linear function $\phi$ and to perform a PCA in this high
dimensional space. %
...
kernel. %
The other trick used is known as the ``representers theorem'' and the interested
reader is referred to \citet{scholkopf_generalized_2001}.

kPCA is very flexible and there exist many kernels for special
...
be seen as special cases of kPCA and an out-of-sample extension using the Nystöm
...
Kernel PCA in R is implemented using the function
\code{kernlab::kpca}, and supports a number of kernels and
user defined functions, for details see \code{?kernlab::kpca}.

The \CRANpkg{dimRed} package wraps around \code{kernlab::kpca} but additionally
provides forward and inverse \citep{bakir_learning_2004} methods which can be
...
\citet{torgerson_multidimensional_1952} and uses an eigenvalue decomposition of
...
Then it tries to find and embedding $Y$ so that $\hat d_{ij}$ is as similar to
$d_{ij}$ as possible.

The disadvantage is that is computationally much more demanding, i.e.\
an eigenvalue decomposition of a $n\times n$ matrix has to be computed
which requires $\mathcal{O}(n^2)$ memory and $\mathcal{O}(n^3)$
...
The \CRANpkg{dimRed} package wraps around \code{cmdscale} and allows the specification
...
graph and classical scaling is used to find an embedding in less
dimensions. This leads to an ``unfolding'' of possibly convoluted
structures (see fig.~\ref{fig:knn}).

Isomap's computational cost is dominated by the eigenvalue decomposition and
therefore scales with $\mathcal{O}(n^3)$. %
the distance matrix, other related techniques can use more efficient algorithms
because the distance matrix becomes sparse due to a different preprocessing.

In R Isomap is implemented in the function \code{vegan::isomap} and the
calculation of geodesic distances in \code{vegan::isomapdist}. %
The \CRANpkg{dimRed} package uses its own implementation which is faster mainly
due to using a KD-tree for the nearest neighbor search (from the \CRANpkg{RANN}
package) and a faster implementation for the shortest path search in the $k$-NNG
(from the \CRANpkg{igraph} package). %
The implementation in \CRANpkg{dimRed} also includes a forward method that can
be used to train the embedding on a subset of data points and then use these
points to approximate an embedding for the remaining points, this technique is
...
reconstructed through linear combinations of their neighborhood if the
manifold is well sampled and the neighbohood lies on a locally linear
patch. %
These reconstruction weights $W$ are the same in the high dimensional
space and the internal coordinates of the manifold. %
...
\end{equation}
is minimized under the constraints that $w_{ij} = 0 $ if $x_j$ does not belong
to the neighborhood and that $\sum_{j=1}^n w_{ij} = 1$. %
...
\end{equation}
which can be solved using an eigenvalue decomposition.
...
In R LLE is implemented by the function \code{lle::lle}, which
unfortunately does not make use of the sparsity of the weight matrix
$W$. %
...
Then a similarity matrix $W$ is calculated by using a Gaussian kernel (see eq.
...
distance. %
The degree of vertex $i$ is $d_i = \sum_{j=1}^n w_{ij}$ and the degree matrix is
the matrix $D$ is the matrix with the entries $d_i$ in the diagonal. %
Then we can form the graph Laplacian $L = D - W$ and there are several ways how
to proceed, an overview can be found in \citet{luxburg_tutorial_2007}. %

The \CRANpkg{dimRed} package implements the algorithm from
\citet{belkin_laplacian_2003}, analogously to LLE, Laplacian eigenmaps
avoids computational complexity by creating a sparse matrix and not
...
Diffusion Maps \citep{coifman_diffusion_2006} takes a distance matrix
...
Both algorithms depart from the same weight matrix, Diffusion Map
...
Isomap (see bottom right fig. \ref{fig:knn}). %
...
The implementation in \CRANpkg{dimRed} is based on the
\code{diffusionMap::diffuse} function.
...
R implements nMDS by \code{MASS::isoMDS} and
\code{vegan::monoMDS}, related methods include Sammons Mapping which
con be found as \code{MASS::sammon}. %
The \CRANpkg{dimRed} package wraps around \code{vegan::monoMDS}.
...
Force directed algorithm see the edges of the graphs as springs or the result of
...
Graph embedding algorithm generally suffer from long running times (compared to
...
of the shortcomings, e.g.\ the Kamada-Kawai \citep{kamada_algorithm_1989}, the
...
\code{igraph} package, they can be accessed using the
\code{igraph::layout\_with\_*} function family. The \CRANpkg{dimRed} package
...
similarities of the points $i$ and $j$ in high dimensional space, $p_{ij}$, and low
dimensional space, $q_{ij}$:

\begin{equation}\label{eq:kldivergence}
    KL(P \| Q) = \sum_{i\neq j} p_{ij}\log\frac{p_{ij}}{q_{ij}}.
\end{equation}
SNE uses a Gaussian kernel (see eq.~\ref{eq:gauss}) to compute similarities in
high and low dimensional space. $t$-Distributed Stochastic Neighborhood
...
$t$-SNE has one parameter, perplexity, to tune which determines the
neighborhood size of the kernels used.

The general runtime of $t$-SNE is $\mathcal{O}(n^2)$, but an efficient
implementation using tree search algorithms that scales $\mathcal{O}(n\log n)$
exists and can be found in the \code{Rtsne} package in R. %
The $t$-SNE implementation in \CRANpkg{dimRed} wraps around the \CRANpkg{Rtsne}
package.
...
several microphones and tries to ``un-mix'' them and find the original
...
FastICA maximizes a measure for nongaussianity called negentropy $J$
\citep{comon_independent_1994}, which is equivalent to minimizing
...
structure as $u$. FastICA uses a very efficient approximations to calculate
negentropy. Because ICA can be translated into a simple linear projection, a
forward an inverse method can be supplied.

There are a number of packages in R that implement algorithms for
ICA, the \CRANpkg{dimRed} package wraps around the
\texttt{fastICA::fastICA()} function.
...
The use of KRR also has the advantage of making the method convex, here we list
it under non-convex methods, because other types of regression may make it non-convex.

Mathematicaly functions are limited to map one input to a single output point,
...
The advantage of unsupervised learning is that one does not need to specify
classes or a target variable for the data under scrutiny. Instead the chosen
algorithm arranges the input data e.g.\ into clusters or a lower dimensional
representation. %
In contrast to a supervised problem, there is no natural way to directly measure
the quality of any output or to compare two methods by an objective measure like
for instance modeling efficiency or classification error. %
The reason is that every method optimizes a different error function, and it
would be unfair to compare $t$-SNE and PCA by means of either recovered variance
or KL-Divergence. %
One fair measure would be the reconstruction error, i.e.\ reconstructing the
original data from a limited number of dimensions, but as shown above not many
...
The \CRANpkg{dimRed} package provides a number of quality measures which have
been proposed in literature to measure performance of dimensionality
...
changes in ordinal distance, just as before, let $d_{ij} = d(x_i,x_j)$
...
wise by their ranks. %
This means, that $r_{ij}$ is an integer which indicates that $x_i$ is the
...
\end{equation}
which is the 2d-histogram of the ranks, i.e.\ $q_{ij}$ is an integer which counts
...
far away points onto each other and if most of the non-zero entries are in the
...
the values a in the lower triangle, then the embedding collapses the original
...
This quantity can be adjusted for random embeddings, which gives the Local
...
maximum at $k_{\max} $. %
Two measures without parameters can be defined, then:
...
\begin{align}\label{eq:qlocalglobal}
  ...
  Q_{\text{global}} &= \frac{1}{n - k_{\max}} \sum_{k=k_{\max}}^{n-1} 
                      Q_{N\!X}(k),
\end{align}
which measure the preservation of local and global distances respectively, the
...
The \CRANpkg{dimRed} package contains the functions \code{Q\_local},
\code{Q\_global}, \code{Q\_NX}, \code{LCMC}, and \code{R\_NX} to
calculate the above quality measures and \code{AUC\_lnK\_R\_NX}. %

Calculating the co-ranking matrix is a relatively expensive operation
because it requires sorting every row or the distance matrix twice
and therefore scales with $\mathcal{O}(n^2\log n)$. %
...
legend (see fig. \ref{fig:plotexample}). %
...
matrices, hence the name) in high and low dimensional space. Additionally the
distance measure and correlation method can be varied. %
In the \CRANpkg{dimRed} package this is implemented in the
\code{cophenetic\_correlation}. %
...
reduction if the method provides a inverse mapping is 
the reconstruction error.
The \CRANpkg{dimRed} package includes a function to calculate the root mean
squared error which is defined as:
\begin{equation}\label{eq:rmse}
\text{RMSE} = \sqrt{\frac{1}{n} \sum_{i=1}^n {d(x'_i, x_i)}^2}
\end{equation}
with $x'_i = f^{-1}(y_{i})$, and $f^{-1}$ being the
function that maps an embedded value back to feature space. %

The \CRANpkg{dimRed} package provides the \code{reconstruction\_rmse} and
...
dimensionality reduction technique. Common ones being the 3d S-curve, the Swiss
roll among others. %
...
The \CRANpkg{dimRed} package implements a number of test datasets that are being
...
The \CRANpkg{dimRed} package collects DR methods readily implemented in R,
implements missing methods and offers means to compare the quality of
embeddings. %
The package is open source and available under the GPL3 license on github
(\url{https://github.com/gdkrmr/dimRed}) and CRAN
(\url{https://cran.r-project.org/package=dimRed}). %
\CRANpkg{dimRed} provides a common interface and convenience functions for a variety of
different DR methods so that it is made easier to use and compare different
methods. An overview can be found in table~\ref{tab:interfacefun}. %

\begin{table}[h]
    \caption[Main interface functions.]{The main interface functions
      of the \CRANpkg{dimRed} package.}\label{tab:interfacefun}
    \centering
    \begin{tabular}{l p{8cm}}
      ...
      \code{plot}              & Plot a \code{dimRedData} or
                                 ...
                                 exploring the data. \\
      \code{plot\_R\_NX}       & For comparing the quality of various
                                 embeddings. \\
...
system in R (s.\ table~\ref{tab:s4classes}). %
The package contains simple conversion functions from and to standard
R-objects like \code{data.frame} and \code{matrix}. %
The \code{dimRedData} class provides an container for the data to be
processed. %
The slot \code{data} contains a \code{matrix} with dimensions in columns and
observations in rows, the slot \code{meta} may contain a \code{data.frame} with
additional information, e.g.\ categories or other information of the data
points. %

\begin{table}[h]
    \caption[S4 Classes]{The S4 classes used in the
      \CRANpkg{dimRed} package}\label{tab:s4classes}
    \centering
...
\code{list} of standard parameters. %
...
take standard R objects like \code{data.frame}, \code{matrix}, or \code{formula}
as input which are automatically coerced to the internal S4 classes. %
The \code{method} is given as a character vector, all available methods can be
listed by calling \code{dimRedMethodList()}. %
Method specific parameters in \code{\ldots} can be given, if no method specific
parameters are given defaults are chosen. %
\code{embed} returns an object of class \code{dimRedResult}

For comparing different embeddings \CRANpkg{dimRed} contains the function
...
of all quality functions is returned by calling \code{dimRedQualityList()}. %

For easy visual examination the package contains \code{plot} functions for
\code{dimRedData} and \code{dimRedResult} objects in order to plot high
dimensional data like parallel plots and pairwise scatter plots. %
Automatic coloring of data points is done using the meta data. %

\section{Examples}
\label{sec:examples}

The comparison of different DR methods, choosing the right parameters for a
method and the inspection of the results is made very simple by
\CRANpkg{dimRed}. %
This section contains a number of examples to highlight the use of the package. %
...
e.g.\ by defining a character vector of method names and calling
\code{lapply}. %
For inspection \CRANpkg{dimRed} provides methods for the \code{plot} function to
visualize the resulting embedding (fig.~\ref{fig:plotexample} b and d), internal
...
Therefore we can see from fig.~\ref{fig:plotexample}c that $t$-SNE is very good a
maintaining close and medium distances for the given data set, PCA is only
...
the interface of \CRANpkg{dimRed} can be used to facilitate searching the
parameter space.

Isomap (see sec.~\nameref{sec:isomap}) has one parameter $k$ which determines
the number of neighbors used to construct the $k$-NNG\@. %
If this number is too large, then Isomap will resemble an MDS
(fig.~\ref{fig:knn} e), if the number is too small, the resulting embedding
contains holes (fig.~\ref{fig:knn} c). %
The following code finds the optimal value, $k_{\text{max}}$, for $k$ using the
$Q_{\text{local}}$ criterion, the results are visualized in fig.~\ref{fig:knn}
a:
...
@
    ...
    \caption[estimating $k$ using @Q_\text{local}]{%
      Using \CRANpkg{dimRed} and the $Q_\text{local}$ indicator to estimate a
      ...
      (c) $k = 5$, the value for $k$ is too small resulting in holes in the
...
Fig.~\ref{fig:knn}a shows how the $Q_{\text{local}}$ criterion changes when
varying the neighborhood size $k$ for Isomap, the gray lines in
fig.~\ref{fig:knn} represent the edges of the $k$-NN Graph. %
If the value for $k$ is too low, the inner structure of the manifold will still
be recovered, but it will be imperfect (fig.~\ref{fig:knn}c, note that the holes
...
The short circuiting can be observed in fig.~\ref{fig:knn}e with the edges that
...
\code{dimRedMethodList} returns a character vector with all methods, a
visualization of the matrix can be found in fig.~\ref{fig:qualityexample}. %
...
for (e in embed_methods) {
    embedded_data[[e]] <- embed(iris_data, e)
...
@
  ...
  \caption[Quality comparision]{%
    ...
    be seen as a general quality assessment of methods. %
...
This showcases the simplicity with which different methods and quality criteria
can be combined. %
...
There is no automatized way to tune parameters in \CRANpkg{dimRed}. %

\section{Conclusion}
\label{sec:conc}

This paper presented the \CRANpkg{dimRed} and \CRANpkg{coRanking} packages and
provides a brief overview of the methods implemented therein. %
\CRANpkg{dimRed} is written in the R language, which is one of the most popular
languages for data analysis and is freely available through the built in package
management system CRAN. %
It is object oriented and completely open source and therefore easily available
and extensible. %
Although most of the DR methods already had implementations in R,
\CRANpkg{dimRed} adds some new methods for dimensionality reduction, and
\CRANpkg{coRanking} adds methods for an independent quality control of DR
methods to the R ecosystem. %
DR is a widely used technique but due to the lack of easily usable tools,
choosing the right method is complex and depends a variety of factors. %
The \CRANpkg{dimRed} package aims to facilitate experimentation with different
techniques, parameters, and quality measures so that choosing the right method
becomes easier. %
\CRANpkg{dimRed} wants to enable the user to objectively compare methods that

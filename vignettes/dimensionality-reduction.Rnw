...
with $q \ll p$. %
Some methods provide an explicit mapping $f(x'_i) = y_i$. Some even offer a
...
be seen as special cases of kPCA. In which case, an out-of-sample extension using the Nystöm
...
reduction when the method provides a inverse mapping is 
